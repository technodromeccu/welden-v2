        ...
        <Card className="border border-outline-variant/15 shadow-sm">
          ...
          <CardContent className="space-y-3 text-sm text-secondary">
            <div className="rounded-xl bg-surface-container-low p-4">
              <div className="text-xs font-medium text-secondary">Default assignee</div>
        ...
        <Card className="border border-outline-variant/15 shadow-sm">
          ...
          <CardContent className="space-y-4 text-sm text-secondary">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-xl bg-surface-container-low p-4"><div className="text-xs font-medium text-secondary">Last success</div><div className="mt-2 text-base font-semibold text-on-surface">{backupStatus?.lastSuccessAt ? fmtDate(backupStatus.lastSuccessAt) : "Not run yet"}</div></div>
              <div className="rounded-xl bg-surface-container-low p-4"><div className="text-xs font-medium text-secondary">Last failure</div><div className="mt-2 text-base font-semibold text-on-surface">{backupStatus?.lastFailureAt ? fmtDate(backupStatus.lastFailureAt) : "No failures recorded"}</div></div>
              <div className="rounded-xl bg-surface-container-low p-4"><div className="text-xs font-medium text-secondary">Latest snapshot</div><div className="mt-2 break-all text-base font-semibold text-on-surface">{backupStatus?.latestSnapshotName ?? "No snapshot yet"}</div></div>
              <div className="rounded-xl bg-surface-container-low p-4"><div className="text-xs font-medium text-secondary">Latest secrets backup</div><div className="mt-2 break-all text-base font-semibold text-on-surface">{backupStatus?.latestSecretsName ?? "No secrets backup yet"}</div></div>
            </div>
            <div className="rounded-xl bg-surface-container-low p-4">
              <div className="text-sm font-semibold text-on-surface">Next scheduled runs (UTC)</div>
            ...
            <div className="rounded-xl bg-surface-container-low p-4">
              <div className="text-sm font-semibold text-on-surface">Recent Google Drive artifacts</div>
              <div className="mt-3 space-y-2">
            ...
            <div className="rounded-xl bg-surface-container-low p-4">
              <div className="text-sm font-semibold text-on-surface">Local admin downloads</div>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-3 md:grid-cols-2">
            <label className="grid gap-2 text-sm">
              <span className="text-xs font-medium text-secondary">Default assignee</span>
            ...
            <label className="grid gap-2 text-sm">
              <span className="text-xs font-medium text-secondary">Business days</span>
            ...
            <label className="grid gap-2 text-sm"><span className="text-xs font-medium text-secondary">Business hour start</span><Input type="number" min="0" max="23" value={String(settingsDraft.businessHours.start)} onChange={(e) => setSettingsDraft((current) => ({ ...current, businessHours: { ...current.businessHours, start: Number(e.target.value) } }))} /></label>
            <label className="grid gap-2 text-sm"><span className="text-xs font-medium text-secondary">Business hour end</span><Input type="number" min="1" max="24" value={String(settingsDraft.businessHours.end)} onChange={(e) => setSettingsDraft((current) => ({ ...current, businessHours: { ...current.businessHours, end: Number(e.target.value) } }))} /></label>
            <label className="grid gap-2 text-sm"><span className="text-xs font-medium text-secondary">First-response SLA days</span><Input type="number" min="1" value={String(settingsDraft.firstResponseSlaWorkingDays ?? 2)} onChange={(e) => setSettingsDraft((current) => ({ ...current, firstResponseSlaWorkingDays: Number(e.target.value) }))} /></label>
            <label className="grid gap-2 text-sm"><span className="text-xs font-medium text-secondary">Reminder lead hours</span><Input type="number" min="1" value={String(settingsDraft.slaReminderLeadHours ?? 4)} onChange={(e) => setSettingsDraft((current) => ({ ...current, slaReminderLeadHours: Number(e.target.value) }))} /></label>
            <label className="grid gap-2 text-sm"><span className="text-xs font-medium text-secondary">Escalation lead hours</span><Input type="number" min="1" value={String(settingsDraft.slaEscalationLeadHours ?? 24)} onChange={(e) => setSettingsDraft((current) => ({ ...current, slaEscalationLeadHours: Number(e.target.value) }))} /></label>
            <label className="grid gap-2 text-sm"><span className="text-xs font-medium text-secondary">Stale lead threshold (days)</span><Input type="number" min="1" value={String(settingsDraft.staleLeadDays ?? 5)} onChange={(e) => setSettingsDraft((current) => ({ ...current, staleLeadDays: Number(e.target.value) }))} /></label>
            <div className="grid gap-2 text-sm md:col-span-2"><span className="text-xs font-medium text-secondary">Internal notification emails</span><ArrayTextarea splitBy="comma" value={settingsDraft.internalNotificationEmails ?? []} onChange={(val) => setSettingsDraft((current) => ({ ...current, internalNotificationEmails: val }))} /></div>
            <div className="grid gap-2 text-sm md:col-span-2"><span className="text-xs font-medium text-secondary">Escalation emails</span><ArrayTextarea splitBy="comma" value={settingsDraft.slaEscalationEmails ?? []} onChange={(val) => setSettingsDraft((current) => ({ ...current, slaEscalationEmails: val }))} /></div>
            <div className="grid gap-2 text-sm md:col-span-2"><span className="text-xs font-medium text-secondary">Quotation CC emails</span><ArrayTextarea splitBy="comma" value={settingsDraft.quotationCcEmails ?? []} onChange={(val) => setSettingsDraft((current) => ({ ...current, quotationCcEmails: val }))} /></div>
            <div className="grid gap-2 text-sm md:col-span-2"><span className="text-xs font-medium text-secondary">Chatbot quick action questions</span><ArrayTextarea splitBy="newline" placeholder="What is the price of the Automatic Pipe Cutting Machine?" value={settingsDraft.quickActionQuestions ?? []} onChange={(val) => setSettingsDraft((current) => ({ ...current, quickActionQuestions: val }))} /></div>
            <label className="grid gap-2 text-sm md:col-span-2">
              <span className="text-xs font-medium text-secondary">Quotation brand name</span>
            ...
            <div className="grid gap-3 text-sm md:col-span-2">
              <span className="text-xs font-medium text-secondary">Quotation logo</span>
              ...
              <div className="rounded-xl border border-outline-variant/12 bg-white p-4">
                <div className="text-xs font-medium text-secondary">Logo preview</div>
